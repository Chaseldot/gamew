...
## Round 2 appended open tickets

- ID: WUX-011
  Source reviewer: worker-1 / Senior Wuxia Fan
  Impacted layer: 流派边界
  Impacted canon file / section: `docs/design/01-main-disciplines.md` / `## 游锋`、`## 奇门`; `docs/design/05-identity-and-archetypes.md` / `## Archetype build grammar`
  Problem: `铁伞书生型` 暴露一类可见 hybrid 道具边界。当前 canon 已分别写清游锋软兵 / 兵刃表现、奇门器具载体与射艺远端触发，但没有明确说明 `伞 / 扇 / 拂尘鞘 / 手杖` 这类既像兵刃又像器具的外显道具在当前 round 默认归谁。结果是玩家容易把一把铁伞同时读成近身护身兵刃、机关壳体、暗器容器与布置媒介，偷渡多主轴功能并模糊 counterplay。
  Expected revision: 补一条 hybrid visible prop 归位规则：这类道具在当前 round 必须先落成一个主轴表达，例如游锋的近身兵刃表现，或奇门的器具 / 载体表现；射艺、影踪只能作为旁修接口补触发或切入。需要明确它不能同时主张护身壳、远端连发、布置载体和主闭环，也不能借 `LOADOUT_INTERFACE_PENDING` 模糊带过。
  Pass condition: 读者不新增 `伞法` 流派、不诉诸未定 loadout，也能清楚解释 `铁伞书生型` 由哪个主修 / 传承成立、哪些只是旁修接口与表现标签，并能指出可读 counterplay。
  Priority: P1
  Status: resolved
  Resolution note: 已在 `docs/design/01-main-disciplines.md` 与 `docs/design/05-identity-and-archetypes.md` 增补 hybrid visible prop 归位规则，并补入铁伞书生示例：`伞 / 扇 / 手杖` 等必须先声明为近身兵刃或器具载体之一，射艺 / 影踪只能补接口，不能一件道具同时领取护身、连发、布置与双主轴闭环。Tradeoff：保留可见 hybrid 造型，但强制单主轴归位。 Rejected：新增 `伞法` 流派 | 会把道具外形误升为职业根；借 `LOADOUT_INTERFACE_PENDING` 模糊处理 | 会制造隐藏功能堆叠与反制不清。

- ID: WUX-012
  Source reviewer: worker-1 / Senior Wuxia Fan
  Impacted layer: CRPG玩法 / counterplay
  Impacted canon file / section: `docs/design/05-identity-and-archetypes.md` / `### Companion and named-organization boundary`; `docs/design/02-inheritances.md` / `## 奇门 -> ### 机关`
  Problem: `牵丝傀儡师型` 把 companion 问题换成了 puppet / minion 问题。当前 `机关` 边界已经拒绝“召唤流工程师”，但角色形象层还没把“可见傀儡道具 / 诱饵 / 假人演出”与“独立参战傀儡”切开。若不补这层，玩家会误以为机关、符禁、药烟或影踪接口可以支撑傀儡独立移动、夹击、承伤、占格、接力触发与行动经济。
  Expected revision: 明确 `傀儡 / 机关偶 / 蛊偶 / 牵丝木人` 边界：静态假人、单次诱敌、机关载体、剧情 / NPC 演出或一次性陷阱媒介可以作为角色形象 / 器具表现；任何独立战斗单位、共享回合、代替承伤、占格换位、接力攻击或资源 relay 都必须 deferred / escalated 到 future minion / puppet system，而不是当前 round 的奇门或旁修补丁。
  Pass condition: 读者能明确区分“演出 / 诱饵 / 载体版傀儡可 pass”与“独立参战 puppet 版必须 block / defer”，并且不会再把 `机关` 读成可偷渡 minion system 的主修。
  Priority: P1
  Status: resolved
  Resolution note: 已在 `docs/design/02-inheritances.md` / `## 奇门 -> ### 机关` 与 `docs/design/05-identity-and-archetypes.md` 明确 puppet 边界：静态假人、诱饵、机关载体、剧情演出与一次性陷阱媒介可 pass；任何独立移动、占格、承伤、夹击、接力触发或共享行动经济的傀儡都 deferred 到 future minion / puppet system。Tradeoff：保留奇门机关的可见器具 / 陷阱幻想，但拒绝独立战斗单位。 Rejected：把机关偶当伪 companion 补丁 | 会偷渡 minion system 并破坏当前 counterplay。

## Round 2 CRPG reviewer append-only advice

- ID: CRPG-R2-ADVICE-01
  Source reviewer: worker-3 / Round 2 CRPG reviewer
  Scope: WUX-011 post-resolution guardrail
  Advice: 当前 hybrid visible prop 的规则已经落在正确层级：`01-main-disciplines.md` 负责主轴 ownership，`05-identity-and-archetypes.md` 负责 build grammar。除非 future fantasy 明确证明现有两条合法读取都缺主循环，否则不要把 `伞 / 扇 / 手杖` 再拆成新传承、专属旁修或 loadout 特例。
  Why: 再往 `02` 或 `03` 里追加同题文本，会把道具外形重新抬升为系统根，并制造“某旁修必绑才能把造型玩完整”的隐性税。
  Priority: P2
  Status: advisory / no rewrite required
  Resolution note: Canon Writer second pass preserved the current WUX-011 canon packet without edits and kept this advice as an active future guardrail only.

- ID: CRPG-R2-ADVICE-02
  Source reviewer: worker-3 / Round 2 CRPG reviewer
  Scope: WUX-012 future-system escalation rule
  Advice: 未来凡是提到独立移动、占格、拦截、承伤、夹击、接力触发、资源 relay 或共享回合的 puppet / minion fantasy，都应直接开 future-system ticket，不再回到 `机关`、`符禁`、`药烟` 或 `影踪` 文本里找局部补丁。
  Why: 当前 round 已经把“器具 / 诱饵 / 演出版傀儡”和“独立战斗单位版傀儡”切开；继续在现有 canon 层补缝，只会把奇门的可读 counterplay 与 action economy 再次搅混。
  Priority: P1
  Status: advisory / future-system only
  Resolution note: Canon Writer second pass preserved the current WUX-012 canon packet without edits and confirmed that future combat-puppet asks escalate out of current canon.
