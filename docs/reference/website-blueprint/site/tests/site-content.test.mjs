...
test('stories page mounts classic route showcase content and story artwork', async () => {
  ...
  assert.ok(data.classicRouteStory.sections.some((section) => section.image?.includes('art-assets/01-第一章-山水县境')));
...
test('art gallery exposes the new art-assets taxonomy', async () => {
  const data = await loadData();
  ...
  assert.ok(data.gallery.length >= 40, 'gallery should expose the expanded art-assets library');
