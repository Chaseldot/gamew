...
## Round 2 addendum — WUX-011 / WUX-012 post-rewrite review
- Review date：2026-04-25
- Review scope：current git diff plus `docs/design/01-main-disciplines.md`、`docs/design/02-inheritances.md`、`docs/design/03-side-studies-and-loadout.md`、`docs/design/05-identity-and-archetypes.md`、`docs/working/fantasy-test-suite.md`、`docs/working/revision-tickets.md`、`docs/working/round-report-latest.md`
- Focus：`铁伞 / 扇 / 手杖` 等 hybrid visible prop 的单主轴归位、`傀儡 / 机关偶 / 牵丝木人` 的 minion boundary，以及这些改动是否重新引入 mandatory tax、隐藏 action economy 或 role-image flattening。
- Gate snapshot：当前 writer first pass 已把 WUX-011 落在主修边界与 archetype grammar，把 WUX-012 落在 `奇门 -> 机关` 与 role-image boundary；`03-side-studies-and-loadout.md` 未被扩写成新的道具专精或 puppet 补丁，placeholder 协议保持不变。
- Verdict：**No P0/P1 mandatory canon rewrite is required for Canon Writer second pass.** 当前改法已经把风险压回可接受区间；second pass 以保边界、补审计记录为主，不需要再开主文档大改。

### Round 2 targeted verdict table

| Check | Current support | Risk | Ticket ID | Verdict |
|---|---|---|---|---|
| build diversity | `游锋` 与 `奇门` 各自提供一条合法的铁伞 / 扇类读取，且都能在不新增 `伞法` 的前提下成立 | 若后续把 visible prop 继续细拆成新传承 / 新旁修，会把造型重新升格为职业根 | WUX-011 | PASS |
| counterplay readability | 游锋版 hybrid prop 仍回到贴身断招 / 缠压 / 藏锋反制；奇门版仍回到拆器具 / 绕布置 / 抢触发窗口；puppet 独立参战已明确 deferred | 若 future 写法又允许同一道具同时兼任护身壳、连发容器与布置媒介，反制会再次变糊 | WUX-011, WUX-012 | PASS |
| action economy | `机关` 明确拒绝独立移动、占格、承伤、夹击、接力触发与共享行动经济的 puppet 版本，避免当前 round 平白多出第二战斗单位 | future 若把诱饵 / 载体版傀儡继续加上拦截或 soak 语句，会重新踩回 minion system | WUX-012 | PASS |
| 2-4 turn tactical rhythm | hybrid prop 现在继承所属主修的现有节奏：游锋走贴近→滚势→收束，奇门走布置→诱发→回收；没有额外 setup 税 | future 若把 prop 写成必须同时带远端触发与近身收束，可能重新出现 hidden setup tax | WUX-011 | PASS |
| mandatory tax pick risk | 新规则把 ownership 放在主修边界与 archetype grammar，而不是塞进 `03` 新开专精，因此没有形成“铁伞必须绑射艺 / 影踪”的 side-study 税 | future 若把 hybrid prop 专门写进旁修梯度，会让可见造型变成必绑接口 | WUX-011 | PASS |
| role-image flattening | `铁伞书生` 仍是组合结果，`傀儡师` 也被拆成可 pass 的器具 / 演出包装与必须 deferred 的独立战斗单位 | future 若继续用称谓驱动战斗规则，会把 archetype grammar 再次压扁 | WUX-011, WUX-012 | PASS |

### Round 2 actionable advice for Canon Writer second pass
- **Do not open a mandatory rewrite ticket for the current WUX-011 / WUX-012 text.** The current ownership split already solves the CRPG blocker without creating a new school, mandatory side-study tax, or hidden minion action economy.
- Keep hybrid visible prop ownership where it is now：`01-main-disciplines.md` defines main-loop ownership, `05-identity-and-archetypes.md` defines build grammar and packaging. Do not migrate this into new `传承` or `03-side-studies-and-loadout.md` text unless a future fantasy proves a missing main-loop.
- Keep puppet / minion escalation where it is now：`02-inheritances.md` carries the `机关` mechanical boundary, `05-identity-and-archetypes.md` carries the role-image boundary. Do not try to rescue combat puppets with `符禁` / `药烟` / `影踪` side interfaces.
- If a future fantasy asks for `铁扇公子`、`铁伞书生`、`杖中藏器` 等同类造型， first ask one question only：它要的是近身兵刃闭环，还是器具布置闭环。先定主轴，再谈接口；不要先堆功能再回头找归属。
- If a future fantasy asks for `傀儡夹击`、`替身承伤`、`远程 relay`、`自动踩位` 或其他独立单位价值， skip local canon patching and open a future-system ticket directly. 当前 round 不该在 fixed canon 里偷做半套 minion rules。
